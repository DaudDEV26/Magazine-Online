  ...
  return (
    <section className="py-16 bg-[#f8f9fa] dark:bg-[#0f0f1a] min-h-screen transition-colors duration-300">
      <div className="w-full max-w-[1200px] mx-auto px-5 flex flex-col gap-10">
        ...
        <h1 className="font-serif text-3xl text-center text-[#1a1a2e] dark:text-white after:content-[''] after:block after:w-16 after:h-[3px] after:bg-[#e94560] after:mx-auto after:mt-2.5">Browse by Category</h1>

        <div className="flex flex-wrap gap-5">
          {categories.map(c => (
            <div key={c.key} onClick={() => handleCategoryClick(c.key)}
              className={`w-full md:w-[calc(33.333%-14px)] text-center cursor-pointer bg-white dark:bg-[#1a1a2e] p-10 rounded-2xl shadow-[0_4px_15px_rgba(0,0,0,0.08)] border-2 hover:-translate-y-1.5 transition-all duration-300 flex flex-col items-center gap-4 ${selectedCategory === c.key ? 'border-[#e94560]' : 'border-transparent hover:border-[#e94560]'}`}>
              <img src="/images/category.svg" alt={c.name} className="w-20 h-20" />
              <h2 className="font-serif text-xl text-[#1a1a2e] dark:text-white">{c.name}</h2>
              <span className="text-[#888888] dark:text-[#a0a0b8] text-sm">{c.count}</span>
              <button className="px-5 py-2 rounded-full font-semibold border-2 border-[#e94560] text-[#e94560] hover:bg-[#e94560] hover:text-white transition-colors duration-200 text-sm">Browse</button>
            </div>
          ))}
        </div>
        ...
        {selectedCategory && (
          <div id="filtered-category-articles" className="flex flex-col gap-8">
            <h2 className="font-serif text-3xl text-center text-[#1a1a2e] dark:text-white after:content-[''] after:block after:w-16 after:h-[3px] after:bg-[#e94560] after:mx-auto after:mt-2.5">Articles in Category</h2>
            <div className="flex flex-wrap gap-5">
              {visibleArticles.map((a, i) => (
                <article key={i} className="w-full md:w-[calc(33.333%-14px)] bg-white dark:bg-[#1a1a2e] rounded-2xl shadow-[0_4px_15px_rgba(0,0,0,0.08)] overflow-hidden border-2 border-transparent hover:shadow-[0_8px_25px_rgba(0,0,0,0.15)] hover:border-[#e94560] transition-all duration-300 flex flex-col">
