// inkwave-react/src/pages/Trending/page.tsx

const featuredTrending = [
  {
    tag: 'Technology',
    title: 'Quantum Computing Breaks New Barrier',
    excerpt: 'Researchers have successfully maintained quantum coherence for an unprecedented ten minutes at room temperature, paving the way for practical consumer applications.',
    views: '1.2M Views', likes: '45K Likes',
  },
  {
    tag: 'Fashion',
    title: 'The Return of 90s Haute Couture',
    excerpt: 'Runways in Paris and Milan are unexpectedly flooded with retro 90s designs, completely reimagined with modern sustainable fabrics and bold new silhouettes.',
    views: '980K Views', likes: '32K Likes',
  },
  {
    tag: 'Politics',
    title: 'Global Climate Accord Signed',
    excerpt: 'Over 150 nations have officially ratified the new climate agreement, committing to aggressive carbon reduction targets by the year 2030.',
    views: '850K Views', likes: '28K Likes',
  },
];
        ...
        <div className="w-full max-w-[1200px] mx-auto px-5 flex flex-col gap-10">
          ...
          <div className="flex flex-col gap-6">
            {featuredTrending.map((a, i) => (
              <article key={i} className="flex flex-col md:flex-row bg-white rounded-2xl shadow-[0_4px_15px_rgba(0,0,0,0.08)] overflow-hidden border-2 border-transparent hover:border-[#e94560] hover:shadow-[0_8px_25px_rgba(0,0,0,0.15)] transition-all duration-300">
          ...
          <div className="flex flex-col gap-4">
            <h3 className="font-serif text-xl text-center text-[#1a1a2e]">Trending Tags</h3>
            <div className="flex flex-wrap justify-center gap-2.5">
              {trendingTags.map(tag => (
                <button key={tag} className="bg-[#eef0f5] text-[#4a4a6a] px-4 py-1.5 rounded-full text-sm font-semibold cursor-pointer hover:bg-[#e94560] hover:text-white transition-colors duration-300">
                  {tag}
                </button>
              ))}
            </div>
          ...
          <div className="flex flex-col md:flex-row gap-10">
            ...
            <div className="flex-1 flex flex-col gap-4">
              <h3 className="font-serif text-xl text-[#1a1a2e]">Weekly Top 10</h3>
              {top10.map((item, i) => (
                <div key={i} className="flex items-center bg-white p-4 rounded-2xl shadow-[0_4px_15px_rgba(0,0,0,0.08)] hover:translate-x-1.5 hover:border-l-4 hover:border-[#e94560] transition-all duration-300">
                  <div className="font-serif text-2xl font-bold text-[#e0e0e0] w-10 shrink-0">{i + 1}</div>
                  <img src="/images/article.png" alt="Thumb" className="w-14 h-14 rounded-lg object-cover mx-4 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-sm text-[#1a1a2e] mb-1 truncate">{item.title}</div>
            ...
            {/* Most Liked */}
            <div className="flex-1 flex flex-col gap-4">
              <h3 className="font-serif text-xl text-[#1a1a2e]">Most Liked This Month</h3>
              {mostLiked.map((a, i) => (
                <article key={i} className="bg-white rounded-2xl shadow-[0_4px_15px_rgba(0,0,0,0.08)] overflow-hidden border-2 border-transparent hover:shadow-[0_8px_25px_rgba(0,0,0,0.15)] hover:border-[#e94560] transition-all duration-300 flex flex-col">
                  <img src={a.img} alt={a.tag} className="w-full h-44 object-cover" />
                  <div className="p-5 flex flex-col gap-2">
                    <span className="text-[#e94560] text-xs font-semibold uppercase">{a.tag}</span>
                    <h3 className="font-serif text-lg">{a.title}</h3>
                    <p className="text-[#888888] text-sm">{a.excerpt}</p>
