// inkwave-react/src/pages/Dashboard/page.tsx
    ...
    <section className="py-16 bg-[#f8f9fa]">
      <div className="w-full max-w-[1200px] mx-auto px-5 flex flex-col gap-10">
        ...
        <div className="flex justify-between items-center flex-wrap gap-4">
          ...
          <button className="px-6 py-2.5 rounded-full font-semibold bg-[#e94560] text-white hover:bg-[#d83550] transition-colors duration-300 cursor-pointer">
            Publish Report
          </button>
        </div>
        ...
        <div className="flex flex-wrap gap-5">
          {[
            ...
            <div key={i} className="bg-white p-6 rounded-2xl shadow-[0_4px_15px_rgba(0,0,0,0.08)] flex flex-col items-center justify-center text-center flex-1 min-w-[180px]">
              <div className="font-serif text-4xl font-bold text-[#1a1a2e]">{s.number}</div>
              <div className="text-[#4a4a6a] text-xs uppercase tracking-widest mt-1">{s.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-5">
          {actionCards.map((a, i) => (
            <div key={i} className="bg-white p-5 rounded-2xl shadow-[0_4px_15px_rgba(0,0,0,0.08)] flex items-center gap-4 cursor-pointer border border-[#e0e0e0] flex-1 min-w-[180px] font-semibold text-[#1a1a2e] hover:border-[#e94560] hover:text-[#e94560] hover:-translate-y-1 transition-all duration-300">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="h-64 flex items-end justify-around border-b-2 border-[#e0e0e0] pt-5 mt-2">
            {chartData.map(bar => (
              <div key={bar.label} className="flex flex-col items-center w-10 h-full justify-end">
                ...
                />
                <span className="mt-2 text-xs text-[#4a4a6a] font-semibold">{bar.label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="w-full max-w-[600px] relative">
            ...
            <input
              type="text"
              className="w-full py-4 pl-11 pr-5 rounded-full border border-[#e0e0e0] text-base shadow-[0_4px_15px_rgba(0,0,0,0.08)] outline-none bg-white"
          ...
          <div className="bg-white rounded-2xl shadow-[0_4px_15px_rgba(0,0,0,0.08)] overflow-x-auto">
            <table className="w-full min-w-[800px] border-collapse">
              <thead>
                <tr>
                  {['ID', 'Title', 'Category', 'Author', 'Date', 'Status', 'Views'].map(h => (
                    <th key={h} className="px-5 py-4 text-left border-b border-[#e0e0e0] bg-[#eef0f5] text-[#4a4a6a] font-semibold uppercase text-xs tracking-wider">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {filteredRows.map((row, idx) => (
                  <tr
                    key={row.id}
                    className={`border-b border-[#e0e0e0] hover:bg-[#e94560]/5 transition-colors duration-300 ${
                      idx % 2 === 1 ? 'bg-[#eef0f5]/30' : ''
                    }`}
                  >
                    <td className="px-5 py-4 text-sm">{row.id}</td>
                    <td className="px-5 py-4 text-sm">{row.title}</td>
                    <td className="px-5 py-4 text-sm">{row.category}</td>
                    <td className="px-5 py-4 text-sm">{row.author}</td>
                    <td className="px-5 py-4 text-sm">{row.date}</td>
                    <td className="px-5 py-4 text-sm">{row.status}</td>
                    <td className="px-5 py-4 text-sm">{row.views}</td>
                  </tr>
                ))}
              </tbody>
            </table>
